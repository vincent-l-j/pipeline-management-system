...
export default function FileLinks({ pitchId }: FileLinksProps) {
  ...
  const [newFile, setNewFile] = useState<FileLinkFormState>({ file_path: '', label: '', description: '' })
  ...
  function loadFiles(): void {
    api.get<FileLink[]>(`/pitches/${pitchId}/files`).then(({ data }) => setFiles(data))
  }
