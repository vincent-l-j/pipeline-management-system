        ...
        <div className="max-w-2xl">
          {results.total === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {categories.map(([catKey, catConfig]: [string, CategoryConfig]) => (
                <div key={catKey}>
                  <h3 className="text-xs font-semibold text-navy-500 uppercase tracking-wide mb-2 flex items-center gap-2">
                    <span>{catConfig.icon}</span>
                    {catConfig.label}
                    <span className="text-navy-300">({results[catKey as keyof SearchResults].length})</span>
                  </h3>

                  <div className="bg-white rounded-xl border border-navy-100 divide-y divide-navy-50 overflow-hidden">
                    {(results[catKey as keyof SearchResults] as SearchResult[]).map((item: SearchResult) => (
                      <button
                        key={item.id}
