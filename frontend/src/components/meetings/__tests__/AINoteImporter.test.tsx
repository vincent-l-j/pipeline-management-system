...
async function expand(user: ReturnType<typeof userEvent.setup>): Promise<void> {
  await user.click(
    screen.getByRole("button", { name: /Import Meeting Notes/ }),
  );
}
...
describe("AINoteImporter", () => {
  ...
  it("renders the collapsed header for admin users", () => {
    ...
    expect(
      screen.queryByPlaceholderText(/Paste your meeting notes/),
  ...
  it("posts the raw notes and shows parsed results", async () => {
    ...
    expect(apiMocks.post.mock).toHaveBeenCalledWith("/meetings/parse-notes", {
      raw_notes: "Meeting notes text",
    });
    expect(
      screen.getByText("Discussed the CRC bid timeline."),
    ).toBeInTheDocument();
  ...
  it("labels results as the basic parser when ai_parsed is false", async () => {
    ...
    apiMocks.post.mockResolvedValue({
      data: { ...parsedResult, ai_parsed: false },
    });
    render(<AINoteImporter onImport={vi.fn()} />);
    await expand(user);
    await user.type(
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Parsed Results (Basic Parser)"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows an error message when parsing fails", async () => {
    ...
    apiMocks.post.mockRejectedValue({
      response: { data: { detail: "Bad notes" } },
    });
    render(<AINoteImporter onImport={vi.fn()} />);
    await expand(user);
    await user.type(
  ...
  it("calls onImport with parsed data when Apply to Meeting is clicked", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Parse Notes" }));
    await waitFor(() =>
      screen.getByRole("button", { name: "Apply to Meeting" }),
    );
