...
def get_pitch_timeline(
        ...
        events.append(
            {
                ...
                "description": f"Moved from {from_label} to {to_label}"
                + (f" — {h.note}" if h.note else ""),
                "actor": users.get(str(h.changed_by_id), "Unknown")
                if h.changed_by_id
                else "System",
            }
        )
    ...
    # Meetings
    meetings = db.query(Meeting).filter(Meeting.pitch_id == pitch_id).all()
    for m in meetings:
        events.append(
            {
                "type": "meeting",
                ...
                if m.summary and len(m.summary) > 200
                else m.summary or "No summary",
                "meeting_id": str(m.id),
    ...
    # Assessments
    assessments = db.query(Assessment).filter(Assessment.pitch_id == pitch_id).all()
        ...
        events.append(
            {
                ...
                "description": a.rationale[:200] + "..."
                if a.rationale and len(a.rationale) > 200
                else a.rationale or "No rationale",
