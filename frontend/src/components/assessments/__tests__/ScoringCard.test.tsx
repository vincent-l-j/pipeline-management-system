...
describe("ScoringCard", () => {
  it("renders the assessment version", () => {
    render(
      <ScoringCard assessment={baseAssessment} assessorName="Jane Smith" />,
    );
  ...
  it("renders the Park recommendation badge", () => {
    render(
      <ScoringCard
        assessment={{ ...baseAssessment, recommendation: "park" }}
        assessorName="Jane"
      />,
    );
  ...
  it("renders rationale text when present", () => {
    ...
    expect(
      screen.getByText(
        "Strong team with a clear pathway to commercialisation.",
      ),
    ).toBeInTheDocument();
  });
