...
export default function ReportsPage(): ReactNode {
  ...
  const filteredPitches = stageFilter
    ? summary!.pitches.filter((p: PitchSummary) => p.current_stage === stageFilter)
    : summary!.pitches
        ...
        <div className="px-6 py-4 border-b border-navy-100 flex items-center justify-between print:pb-2">
          ...
          >
            ...
            {Object.entries(STAGE_BADGE).map(([key]) => {
              const count = summary!.pitches.filter((p: PitchSummary) => p.current_stage === key).length
              if (count === 0) return null
              return (
                <option key={key} value={key}>
                  {key.replace('_', ' ').replace(/\b\w/g, (c: string) => c.toUpperCase())} ({count})
                </option>
              )
            })}
          </select>
