...
describe("MeetingAttendees", () => {
  ...
  it("shows the add form when the Add button is clicked", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Rozetta Staff" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "External Contact" }),
    ).toBeInTheDocument();
  });
  ...
  it("populates the staff select with users from the API", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "+ Add" }));
    expect(
      screen.getByRole("option", { name: "Alice Staff" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("option", { name: "Bob Staff" }),
    ).toBeInTheDocument();
  });
  ...
  it("calls DELETE when Remove is clicked for an attendee", async () => {
    ...
    expect(apiMocks.delete.mock).toHaveBeenCalledWith(
      "/meetings/1/attendees/a1",
    );
  });
});
