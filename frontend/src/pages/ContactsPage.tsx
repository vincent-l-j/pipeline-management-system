import { useState, useEffect, ChangeEvent, MouseEvent, ReactElement } from 'react'
...
export default function ContactsPage(): ReactElement {
  ...
  const [editForm, setEditForm] = useState<ContactForm>({ name: '', role: '', email: '' })
  ...
  const saveEdit = async (contact: Contact): Promise<void> => {
    ...
    for (const field of ['name', 'role', 'email']) {
      ...
      if (next !== (contact[field as keyof Contact] ?? null)) changes[field] = next
    }
    ...
    setError('')
    try {
      const { data } = await api.patch<Contact>(`/contacts/${contact.id}`, changes)
        ...
        <div className="mb-4 p-4 bg-navy-50 rounded-lg space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            <input
              type="text"
              value={form.role}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setForm((p) => ({ ...p, role: e.target.value }))}
                ...
                <tr key={c.id} className="bg-navy-50/50">
                  <td className="px-4 py-3">
                    <input
                      type="text"
                      value={editForm.name}
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setEditForm((p) => ({ ...p, name: e.target.value }))}
                  ...
                  <td className="px-4 py-3">
                    <input
                      type="text"
                      value={editForm.role}
                      onChange={(e: ChangeEvent<HTMLInputElement>) => setEditForm((p) => ({ ...p, role: e.target.value }))}
