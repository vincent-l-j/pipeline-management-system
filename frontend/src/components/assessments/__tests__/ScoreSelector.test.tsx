...
import { Criterion } from "../AssessmentConfig";
...
describe("ScoreSelector", () => {
  it("renders the criterion label and description", () => {
    ...
    expect(
      screen.getByText("Track record, expertise, and execution capacity"),
    ).toBeInTheDocument();
  });
  ...
  it("renders 5 numbered score buttons", () => {
    ...
    for (let i = 1; i <= 5; i++) {
      expect(
        screen.getByRole("button", { name: new RegExp(i.toString()) }),
      ).toBeInTheDocument();
    }
  });
  ...
  it("shows score label text when a value is set", () => {
    render(
      <ScoreSelector criterion={criterion} value={4} onChange={vi.fn()} />,
    );
  ...
  it("renders the score progress bar", () => {
    const { container } = render(
      <ScoreSelector criterion={criterion} value={3} onChange={vi.fn()} />,
    );
