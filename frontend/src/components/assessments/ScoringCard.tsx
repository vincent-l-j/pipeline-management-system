...
/**
 * Renders a read-only scoring card displaying completed assessment details
 * @param assessment - The assessment data with all scores and metadata
 * @param assessorName - Optional name of the assessor
 * @returns React component displaying the assessment card
 */
export default function ScoringCard({
  assessment,
  assessorName,
}: ScoringCardProps): ReactElement {
  const totalScore = CRITERIA.reduce((sum, c) => sum + ((assessment[c.key as keyof Assessment] as number) || 0), 0)
